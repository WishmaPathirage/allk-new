...
export default function TeacherExamPapersPage({ teacher, toast }) {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      if (newFolderThumbFile) {
        ...
        const path = `folder-thumbnails/exam_folders/${teacher.id}_${Date.now()}.${ext}`;
  ...
  const handleDeleteFolder = async (folder) => {
    ...
    if (!window.confirm(
      hasFiles
        ? `Folder "${folder.name}" has items. Delete folder and all its contents?`
  ...
  const handleSaveEdit = async (folder) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'exam_folders', folder.id), { name: editFolderName.trim(), thumbnailUrl, thumbnailStoragePath });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const sRef       = ref(storage, `exam-papers/${teacher.id}/${folderPath}/${fileName}`);
      ...
      await addDoc(collection(db, 'exam_papers'), {
        ...
        fileUrl: url, storagePath: `exam-papers/${teacher.id}/${folderPath}/${fileName}`,
        folderId: selectedFolder || null,
        folderName: folderObj?.name || null,
        uploadedAt: Timestamp.now(),
      });
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 22, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: 20, boxShadow: '0 1px 5px rgba(0,0,0,.06)' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#111', marginBottom: 16 }}>Add Exam Item</div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea
                ...
                placeholder="Brief description of this exam item…"
                value={description}
            ...
            {itemType === 'quiz' ? (
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>Quiz Link (Google Form) *</label>
                <input
                  style={inputStyle}
                ...
                />
                <div style={{ fontSize: 11.5, color: '#aaa', marginTop: 6 }}>Paste the shareable Google Form link. Students open it in a new tab.</div>
              </div>
            ) : (
              <div style={{ marginBottom: 16 }}>
                ...
                <div
                  ...
                  style={{
                    border: `2px dashed ${drag ? ACCENT : '#ddd'}`,
                    borderRadius: 10, padding: '20px 16px', cursor: 'pointer',
                    textAlign: 'center', background: drag ? `${ACCENT}08` : '#fafafa',
                    transition: 'all .2s',
                  }}
                >
                  {file ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
                      <PaperIcon item={{ itemType: 'pdf' }} />
                    ...
                    <>
                      ...
                      <div style={{ fontSize: 13, color: '#aaa' }}>Drop file here or <span style={{ color: ACCENT, fontWeight: 700 }}>browse</span></div>
            ...
            <button
              ...
              style={{
                ...
                color: '#fff', background: ACCENT, border: 'none', borderRadius: 10,
            ...
            >
              {uploading ? (
                <>
                  ...
                  {itemType === 'quiz' ? 'Adding…' : 'Uploading…'}
                </>
              ) : itemType === 'quiz' ? (
                <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: `${ACCENT}11`, border: `1.5px solid ${ACCENT}33`,
            ...
            {showFolderForm && (
              <div style={{ marginBottom: 14, padding: 14, background: '#fafafa', borderRadius: 10, border: '1px solid #eee' }}>
                <div style={{ marginBottom: 10 }}>
                  ...
                  <input
                    style={inputStyle}
                    placeholder="e.g. 2024 Past Papers"
                    value={newFolderName}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <button
                  ...
                  style={{
                    ...
                    background: viewFolder === null ? `${ACCENT}15` : 'transparent',
                    color: viewFolder === null ? ACCENT : '#555',
                  }}
                >
                      ...
                      {isEdit ? (
                        <div style={{ padding: 12, background: '#fafafa', borderRadius: 10, border: '1px solid #eee' }}>
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button
                              ...
                              style={{ fontFamily: 'inherit', fontSize: 12, fontWeight: 700, color: '#fff', background: ACCENT, border: 'none', borderRadius: 7, padding: '7px 14px', cursor: 'pointer' }}
                        ...
                        <div
                          style={{
                            ...
                            background: isView ? `${ACCENT}15` : 'transparent',
                        ...
                        >
                          {folder.thumbnailUrl
                            ...
                            : (
                              <div style={{ width: 32, height: 32, borderRadius: 6, background: `${ACCENT}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={ACCENT} strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                              </div>
                            )
                          }
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 12.5, fontWeight: 600, color: isView ? ACCENT : '#333', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{folder.name}</div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 5px rgba(0,0,0,.06)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #f4f4f4', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#aaa', marginTop: 2 }}>{displayedMats.length} item{displayedMats.length !== 1 ? 's' : ''}</div>
            </div>
            {viewFolder !== null && (
              <button
                onClick={() => setViewFolder(null)}
          ...
          {displayedMats.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 20px', color: '#ccc' }}>
              ...
              <p style={{ fontSize: 13.5 }}>No exam items here yet.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, color: item.itemType === 'quiz' ? ACCENT : '#ff3c2e', background: item.itemType === 'quiz' ? '#f3effc' : '#fff0ef', borderRadius: 99, padding: '1px 7px', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 7, flexShrink: 0 }}>
                    <a
                      href={item.itemType === 'quiz' ? item.quizUrl : item.fileUrl}
                      target="_blank"
                      rel="noreferrer"
                      style={{
                        width: 30, height: 30, borderRadius: 7,
                        background: item.itemType === 'quiz' ? '#f3effc' : '#e8f8f0',
                        color: item.itemType === 'quiz' ? ACCENT : '#27956b',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none',
                      }}
                      title={item.itemType === 'quiz' ? 'Open quiz' : 'Download'}
                    >
                      {item.itemType === 'quiz'
